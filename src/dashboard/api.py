...
async def get_all_config(token: str = Depends(verify_token)):
    ...
    payload = {
        ...
        "imagen": {
            ...
            # SFW/NSFW 模型配置 - 完整矩阵
            "sfw_model": chat_config.GEMINI_IMAGEN_CONFIG.get("SFW_MODEL_NAME", ""),
            "sfw_edit_model": chat_config.GEMINI_IMAGEN_CONFIG.get("SFW_EDIT_MODEL_NAME", ""),
            "sfw_model_2k": chat_config.GEMINI_IMAGEN_CONFIG.get("SFW_MODEL_NAME_2K", ""),
            "sfw_edit_model_2k": chat_config.GEMINI_IMAGEN_CONFIG.get("SFW_EDIT_MODEL_NAME_2K", ""),
            "sfw_model_4k": chat_config.GEMINI_IMAGEN_CONFIG.get("SFW_MODEL_NAME_4K", ""),
            "sfw_edit_model_4k": chat_config.GEMINI_IMAGEN_CONFIG.get("SFW_EDIT_MODEL_NAME_4K", ""),
            "nsfw_model": chat_config.GEMINI_IMAGEN_CONFIG.get("NSFW_MODEL_NAME", ""),
            "nsfw_edit_model": chat_config.GEMINI_IMAGEN_CONFIG.get("NSFW_EDIT_MODEL_NAME", ""),
            "nsfw_model_2k": chat_config.GEMINI_IMAGEN_CONFIG.get("NSFW_MODEL_NAME_2K", ""),
...
async def update_coin_config(config: CoinConfigUpdate, token: str = Depends(verify_token)):
    ...
    if config.summary_imagen_enabled is not None:
        chat_config.FEEDING_CONFIG["SUMMARY_IMAGEN_ENABLED"] = config.summary_imagen_enabled
        await chat_db_manager.set_global_setting(
            "summary_imagen_enabled", str(config.summary_imagen_enabled).lower()
        )
        updated["summary_imagen_enabled"] = config.summary_imagen_enabled

    if config.summary_imagen_resolution is not None:
        resolution = config.summary_imagen_resolution.strip()
        if resolution not in ("default", "2k", "4k"):
            raise HTTPException(400, "总结配图分辨率只支持 default / 2k / 4k")
