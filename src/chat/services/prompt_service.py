...
class PromptService:
    ...
    def _build_image_model_hint_lines(self) -> List[str]:
        ...
        return [
            ...
            '- image_search 返回后，由你先分析多张参考图并决定哪张或哪几张最适合；原始任务是生成图片时，下一步必须调用 edit_image(image_search_reference_index=... 或 image_search_reference_indexes=[...])。edit_prompt 必须短，只写“保持参考图人物身份、脸、发型、服装、画风不变，仅改动作/场景/构图为……”；不要复述外观、服饰、发色、作品名、画风标签或整套文生图提示词；禁止再调用 generate_image / generate_image_novelai / generate_image_comfyui 纯文生图。原始任务是视频时调用 generate_video(use_reference_image=true, image_search_reference_index=...)。代码不会自动替你选图或硬传图。',
    ...
    def _build_compact_tool_guidance(self) -> str:
        ...
        lines = [
            ...
            "例如‘凡人动漫南宫婉’、‘来自《某作品》的角色’）时，第一步必须调用 image_search(send_to_channel=false, analyze_images=true) 获取多张图片参考；"
