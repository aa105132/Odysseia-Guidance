    ...
    def build_chat_prompt(
        ...
        if default_image_engine == 'comfyui':
            ...
            comfyui_natural_language_protocol = (
                '当底模为 zimage/qwen 时，prompt/negative_prompt 必须使用高密度中文自然语言协议：\n'
                ...
                '   - 主体身份：年龄段、族裔/人种、气质定位、人物关系与主体优先级；\n'
                '   - 面部与发型：脸型、眼鼻唇、肤质细节、发色发型、发丝状态；\n'
                '   - 身体与服饰：身形比例、肢体细节、服装结构、材质纹理、配饰道具；\n'
                '   - 姿态与动作：站坐卧姿、四肢角度、手部动作（单手单物）、重心与受力；\n'
                '   - 场景构图：前景/中景/背景、空间层次、主体占画面比例、视觉引导路径；\n'
                '   - 环境与光影：时间天气、主辅光方向、色温、阴影边缘、反光与轮廓光；\n'
                '   - 镜头参数：焦段、机位高低、拍摄距离、景深虚化、快门定格感；\n'
                'C) 写作方式：使用主谓宾直述句，避免空泛形容词；关键细节写“可见的物理特征”，不要抽象感受词堆叠；\n'
                'D) 逻辑约束：人物动作符合人体工学，服装/道具/背景与时代和场景一致，不写互相矛盾的描述；\n'
                'E) 安全约束：涉及私密部位暴露时在 prompt 前缀添加 nsfw；\n'
                'F) 负面提示词：negative_prompt 同样使用中文自然语言，至少包含画质缺陷、解剖错误、构图错误、手部错误、纹理异常五类排除项。'
            )
