...
4. 使用搜索结果作答时，在回复末尾追加“消息源”小节，列出可直接点击的原始 URL（纯链接）。
5. 严禁编造、篡改或替换来源链接。
"""
                final_injection_content = (
...
**核心任务**:
1. 先给出结论，再给简要依据；不要复述无关噪声。
2. 如果结果存在冲突，明确说明差异并给出你的判断。
3. 你引用了哪些信息，就在最终回复末尾追加“消息源”小节。
4. “消息源”必须包含原始 URL（纯链接），方便用户直接点击；严禁编造或改写链接。
5. 如果搜索结果不足以回答，明确告诉用户“信息不足”，并引导补充关键词。
