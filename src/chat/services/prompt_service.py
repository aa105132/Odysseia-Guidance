...
class PromptService:
    ...
    def _build_image_model_hint_lines(self) -> List[str]:
        ...
        return [
            ...
            "- ⚠️ model_name_override 只在用户明确指定模型名时才传（如用户说用gpt画→传 gpt-image-2）。用户没指定模型时**绝对不要传 model_name_override**，让系统自动选择默认模型（SFW→gpt-image-2，NSFW→SEEDREAM_5_PRO）。禁止自己编造模型名如 gpt-4o-mini。",
    ...
    def _build_compact_tool_guidance(self) -> str:
        ...
        lines = [
            ...
            "例如‘凡人动漫南宫婉’、‘来自《某作品》的角色’）时，第一步必须调用 image_search(send_to_channel=false, analyze_images=true) 获取多张图片参考；image_search 每次只能搜一个人物/角色/主体，禁止 [BATCH]、换行、||、逗号分隔、A和B 或任何多名字混搜；"
            ...
            "原始任务是生成图片时，下一步必须调用 edit_image 或 edit_images_batch(image_search_reference_index=... 或 image_search_reference_indexes=[...])，"
