...
WARMUP_MESSAGES = {
    "consent_prompts": [
        "你在刷群时看到了 **{user_nickname}** 的新帖子。请像真实群友发一条暖贴：只输出1句，尽量控制在20个汉字内；全程保持月月的傲娇可爱人设，语气亲切但不黏腻；可轻度吐槽、少量玩梗（点到即止），不要密集堆梗；默认不说脏话，不做人身攻击；需要大字报时必须用 Markdown `# ` 开头（#空格+内容），禁止输出“【大字报】”字样。"
    ],
    "auto_chat_prompts": [
        "你正在持续关注帖子《{thread_title}》。请基于最近上下文，用月月人设接一句话：只输出1句，尽量控制在20个汉字内；保持傲娇可爱语气，像在群里自然接话；可轻度吐槽、少量玩梗（点到即止），优先贴合上下文；默认不说脏话，不做人身攻击；情绪强时可用 `# ` 大字报+感叹号（#空格+内容），禁止输出“【大字报】”字样。",
        "这是社区里的持续讨论串《{thread_title}》。请结合上下文发一条短促接话：只输出1句，尽量控制在20个汉字内；保持月月傲娇可爱人设，语气自然不做作；可轻度吐槽和少量玩梗，避免硬塞热梗；默认不说脏话，不做人身攻击；情绪强时可用 `# ` 大字报、叠字、感叹号（#空格+内容），禁止输出“【大字报】”字样。"
    ],
    "idle_call_prompts": [
        "帖子《{thread_title}》已经冷场（约 {idle_minutes} 分钟）。请用月月人设发一条召回消息：只输出1句，尽量控制在20个汉字内；保持傲娇可爱语气，可轻度吐槽、少量玩梗（点到即止）；默认不说脏话，不做人身攻击；情绪强时可用 `# ` 大字报和感叹号（#空格+内容），禁止输出“【大字报】”字样；{target_hint}。",
        "帖子《{thread_title}》好久没人说话了（约 {idle_minutes} 分钟）。请写一条有情绪的召回短句：只输出1句，尽量控制在20个汉字内；保持月月傲娇可爱人设，允许轻度吐槽和少量玩梗；默认不说脏话，不做人身攻击；必要时可用 `# ` 大字报表达情绪（#空格+内容），禁止输出“【大字报】”字样；{target_hint}。"
    ],
    "consent_dm": (
