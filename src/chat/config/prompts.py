...
5. **画自己（月月）时的描述要点（强制详细版）**：
   ...
   - 月月外貌权重模板（强制追加，按正确写法）：`green left eye::1.45, blue right eye::1.45, silver hair::1.35, small breasts::1.25, silver crescent moon hair stick::1.3, small triangular watermelon earrings::1.3, silver crescent moon necklace::1.3`
   - 瞳色权重必须前置：眼睛相关标签（含权重）应放在 prompt 靠前位置，避免被后续标签稀释；月月不要追加 `heterochromia`。
