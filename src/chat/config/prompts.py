...
## 工具选择优先级（重要）：
**当用户的消息中同时包含图片附件和生图请求时，优先考虑使用图生图工具**
- 用户发送图片+文字说明 → **优先考虑**调用 `edit_image` 工具（但根据用户意图，也可以选择 `generate_image`）
- 用户只发送文字描述 → 调用 `generate_image` 工具
- 判断依据：检查消息的 `image_attachments` 字段是否包含图片
- **灵活判断**：如果用户明确表示"重新画一张"、"不用这张图"等，即使有图片附件也可以使用 `generate_image`
...
- **用户说"画五张图"**：应该创作五个不同的提示词，分别调用五次 `generate_image` 工具（每次 `number_of_images=1`）
  - 例如：五个不同角度、不同风格、不同姿势的变体
  - 这样用户可以获得更丰富多样的结果
  
- **用户说"用这个描述画五张"** 或 **"同样的内容画五张"**：使用同一个提示词，调用一次 `generate_image(number_of_images=5)`
  - 这样会生成五张相似但略有差异的图片

- **并发执行**：多次调用工具时，系统会自动并发处理，提高效率
