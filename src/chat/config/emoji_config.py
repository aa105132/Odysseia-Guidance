...
EMOJI_MAPPINGS = [
    # --- 基础表情 ---
    (re.compile(r"\<得意\>"), ["<:deyi:1466723621996073072>"]),
    (re.compile(r"\<比心\>"), ["<:bixin:1466723597568704512>"]),
    (re.compile(r"\<达咩\>"), ["<:damie:1466723618460532942>"]),
    (re.compile(r"\<护食\>"), ["<:hushi:1466723656708128879>"]),
    (re.compile(r"\<流汗\>"), ["<:liuhan:1466723665629679636>"]),
    (re.compile(r"\<媚眼\>"), ["<:meiyan:1466723672667717697>"]),
    (re.compile(r"\<抓狂\>"), ["<:zhuakuang:1466723730523816070>"]),
    (re.compile(r"\<自闭\>"), ["<:zibi:1466723733065695313>"]),
    (re.compile(r"\<乖巧\>"), ["<:guaiqiao:1466723629998931968>"]),
    (re.compile(r"\<傲娇\>"), ["<:aojiao:1466721778335482008>"]),
    (re.compile(r"\<没眼看\>"), ["<:meiyankan:1466723674790039723>"]),
    (re.compile(r"\<暴怒\>"), ["<:baonu:1466723585107169413>"]),
    (re.compile(r"\<吃瓜\>"), ["<:chigua:1466723605365784626>"]),
    (re.compile(r"\<鄙视\>"), ["<:bishi:1466723594816983061>"]),
    (re.compile(r"\<求夸\>"), ["<:qiukua:1466723691885760664>"]),
    (re.compile(r"\<大哭\>"), ["<:daku:1466723614744252489>"]),
    (re.compile(r"\<慌张\>"), ["<:huangzhang:1466723650798489667>"]),
    (re.compile(r"\<哈欠\>"), ["<:haqian:1466723636101517343>"]),
    (re.compile(r"\<被窝\>"), ["<:beiwo:1466723590836588700>"]),
    (re.compile(r"\<祈祷\>"), ["<:qidao:1466723689583345714>"]),
    (re.compile(r"\<干饭\>"), ["<:ganfan:1466723628111495168>"]),
    (re.compile(r"\<数钱\>"), ["<:shuqian:1466723706578407531>"]),
    (re.compile(r"\<读信\>"), ["<:duxin:1466723626056417311>"]),
    (re.compile(r"\<投币\>"), ["<:toubi:1466723708902047850>"]),
    (re.compile(r"\<黑客\>"), ["<:heike:1466723639528263801>"]),
    (re.compile(r"\<疑惑\>"), ["<:yihuo:1466723722550579293>"]),
    (re.compile(r"\<生闷气\>"), ["<:shengmenqi:1466723700643729419>"]),
    (re.compile(r"\<比耶\>"), ["<:biye:1466723600827416688>"]),
    (re.compile(r"\<画圈圈\>"), ["<:huaquanquan:1466723654804180992>"]),
    
    # --- 生活/动作 ---
    (re.compile(r"\<爆炸\>"), ["<:baozha:1466723587787587806>"]),
    (re.compile(r"\<不要捡\>"), ["<:buyaojian:1466723602908053711>"]),
    (re.compile(r"\<看戏\>"), ["<:kanxi:1466723660948836540>"]),
    (re.compile(r"\<打游戏\>"), ["<:dayouxi:1466723620234461328>"]),
    (re.compile(r"\<害怕\>"), ["<:haipa:1466723633211904113>"]),
    (re.compile(r"\<黑脸\>"), ["<:heilian:1466723643038892104>"]),
    (re.compile(r"\<喝奶茶\>"), ["<:henaicha:1466723646625288232>"]),
    (re.compile(r"\<红牌\>"), ["<:hongpai:1466723648298811622>"]),
    (re.compile(r"\<举牌\>"), ["<:jubai:1466723658851422249>"]),
    (re.compile(r"\<酷\>"), ["<:ku:1466723662840336616>"]),
    (re.compile(r"\<落叶\>"), ["<:luoye:1466723669953744949>"]),
    (re.compile(r"\<懵\>"), ["<:meng:1466723676614430740>"]),
    (re.compile(r"\<魔法\>"), ["<:mofa:1466723679059578892>"]),
    (re.compile(r"\<OK\>"), ["<:ok:1466723687016169544>"]),
    (re.compile(r"\<秋天\>"), ["<:qiutian:1466723694436024370>"]),
    (re.compile(r"\<撒钱\>"), ["<:saiqian:1466723698554704079>"]),
    (re.compile(r"\<睡觉\>"), ["<:shuijiao:1466723703961292861>"]),
    (re.compile(r"\<我好了\>"), ["<:wohaole:1466723714099056690>"]),
    (re.compile(r"\<休息\>"), ["<:xiuxi:1466723716494004264>"]),
    (re.compile(r"\<学废\>"), ["<:xuefei:1466723719929008335>"]),
