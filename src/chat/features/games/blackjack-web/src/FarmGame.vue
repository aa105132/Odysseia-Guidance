...
type FarmTab = 'shop' | 'bag' | 'upgrades' | 'visits' | 'guide';
...
const activeEffect = ref<{ plot: number; kind: FarmAction; label: string } | null>(null);
...
const tabs: { id: FarmTab; label: string }[] = [{ id: 'shop', label: '种子铺' }, { id: 'bag', label: '仓库' }, { id: 'upgrades', label: '洞天' }, { id: 'visits', label: '串门' }, { id: 'guide', label: '灵草图鉴' }];
...
function chooseTab(value: FarmTab) { tab.value = value; if (value === 'visits') void loadVisits(); }
async function visit(id = '') {
  if (busy.value || pendingRetry.value) return;
  notice.value = ''; activeEffect.value = null;
  await loadFarm(id);
...
function displayEffect(body: FarmActionBody, message: string) {
  if (body.plot_id === undefined) return;
  if (effectTimer) clearTimeout(effectTimer);
  activeEffect.value = { plot: body.plot_id, kind: body.action, label: message };
  effectTimer = setTimeout(() => { activeEffect.value = null; }, 1700);
}
async function act(action: FarmAction, fields: Partial<Omit<FarmActionBody, 'action' | 'request_id'>> = {}, retryBody?: FarmActionBody) {
  ...
  try {
    ...
    displayEffect(body, message);
    playGameSound(action === 'harvest' || action === 'sell' || action === 'steal' ? 'win' : 'click');
    ...
    if (!unknownResult) { closePlant(); closeConfirm(); }
  } finally {
...
function onVisibility() { if (!document.hidden && !busy.value && !pendingRetry.value) void loadFarm(ownerId.value, Boolean(state.value)); }
onMounted(() => {
  void loadFarm();
  ticker = setInterval(() => {
...
onBeforeUnmount(() => {
  ...
  plantDialog.value?.close(); confirmDialog.value?.close();
});
</script>

<template>
  <section class="farm-game" aria-label="灵草洞天" :aria-busy="loading || busy">
    <header class="farm-heading">
      <div class="farm-heading-main"><button class="farm-back farm-button quiet" :disabled="busy || loading || Boolean(pendingRetry)" @click="emit('back')">‹ 游戏大厅</button><div><span class="farm-eyebrow">一方灵田 · 静待花开</span><h2>灵草洞天</h2></div></div>
      <div class="farm-heading-actions"><span class="farm-wallet">灵石 <b>{{ number(ownFarm ? state?.balance ?? profile.balance : profile.balance) }}</b></span><GameTools :profile="profile" :api-call="apiCall" audio-only /><button v-if="!ownFarm" class="farm-button" :disabled="busy || loading || Boolean(pendingRetry)" @click="visit()">回我的灵田</button><button class="farm-button quiet" aria-label="刷新农场" :disabled="busy || loading || Boolean(pendingRetry)" @click="loadFarm()">刷新</button></div>
    </header>
    ...
    <template v-if="state">
      ...
      <div class="farm-layout">
        <main class="farm-garden">
          <div class="garden-caption"><div><span>灵田</span><h3>{{ ownFarm ? '今日，也照料一株生机' : '来邻居的洞天走走' }}</h3></div><span class="garden-weather"><i /> 灵气流转</span></div>
          <div class="farm-plots">
            <article v-for="plot in state.plots" :key="`${state.owner.user_id}-${plot.plot_id}`" class="farm-plot" :class="[`plot-${plot.status}`, { 'plot-has-pest': plot.has_pest && !plot.pest_cleared, 'plot-effect': activeEffect?.plot === plot.plot_id }]" :aria-label="`${plot.plot_id}号灵田 ${plot.crop_name || '空地'} ${plotLabel(plot)}`">
              <header><span class="plot-number">{{ String(plot.plot_id).padStart(2, '0') }}</span><span v-if="plot.has_pest && !plot.pest_cleared" class="plot-badge pest-badge">有虫害</span><span v-else-if="plot.status === 'mature'" class="plot-badge" :class="`quality-${plot.quality}`">{{ qualityNames[plot.quality ?? 'normal'] }}</span><span v-else-if="plot.watered" class="plot-watered">已浇水</span></header>
              <div class="plot-scene" :class="{ 'scene-water': activeEffect?.plot === plot.plot_id && activeEffect.kind === 'water' }">
                <div class="plot-earth"><i /><i /><i /></div>
                <FarmPlant v-if="plot.crop_id" :name="plot.crop_name || ''" :icon="cropIcon(plot.crop_id)" :stage="stage(plot)" :quality="plot.quality" />
                <span v-else class="plot-empty-mark" aria-hidden="true">＋</span>
                <div v-if="activeEffect?.plot === plot.plot_id" class="plot-action-effect" :class="`effect-${activeEffect.kind}`" aria-hidden="true"><i v-for="particle in 7" :key="particle" :style="{ '--particle': particle }" />{{ activeEffect.kind === 'harvest' || activeEffect.kind === 'steal' ? '收入仓库' : activeEffect.kind === 'water' ? '灵露滋养' : activeEffect.kind === 'pest' ? '虫害已除' : '种下希望' }}</div>
              </div>
              <div class="plot-info"><strong>{{ plot.crop_name || '一畦新土' }}</strong><span>{{ plotLabel(plot) }}</span><div v-if="plot.status === 'growing'" class="plot-progress"><i :style="{ width: `${plotProgress(plot)}%` }" /></div><small v-else-if="plot.status === 'mature'">剩余 {{ plot.yield_remaining }} 份<template v-if="plot.stolen_count"> · 已被采 {{ plot.stolen_count }} 份</template></small><small v-else>种下一粒，收获一季</small></div>
              <div v-if="ownFarm" class="plot-actions">
          ...
          <div class="farm-garden-note"><span aria-hidden="true">✦</span> 浇水可缩短本轮等待；有虫也会成熟，及时除虫可保住收成。</div>
          <p v-if="protectionRemaining" class="farm-protection">新手守护还剩 {{ secondsLabel(protectionRemaining) }}，期间灵草不会被偷采。</p>
          <details v-if="state.activity.length" class="farm-activity"><summary>洞天记事 <span>最近 {{ state.activity.length }} 条</span></summary><ol><li v-for="(item, index) in state.activity" :key="index"><span>{{ item.message || '完成一次照料' }}</span><time>{{ activityTime(item.created_at ?? item.timestamp) }}</time></li></ol></details>
        </main>
        <aside class="farm-panel" aria-label="农场经营">
          <nav class="farm-tabs" aria-label="农场功能"><button v-for="item in tabs" :key="item.id" :class="{ active: tab === item.id }" :aria-pressed="tab === item.id" @click="chooseTab(item.id)">{{ item.label }}<small v-if="item.id === 'bag' && inventoryCount">{{ inventoryCount }}</small></button></nav>
...
<style scoped>
.farm-game { --farm-ink: #334b3e; --farm-muted: #788274; --farm-line: #d8daca; --farm-green: #51765a; --farm-gold: #9b7a43; color: var(--farm-ink); color-scheme: light; display: flex; flex-direction: column; flex: 1 1 0; min-width: 0; min-height: 0; overflow: auto; padding: clamp(12px, 2vw, 28px); background: radial-gradient(ellipse at 22% 10%, #f9f5dfba, transparent 60%), linear-gradient(140deg, #e8eddc, #eef0e4 50%, #dce7d8); border: 1px solid #f4f1df; border-radius: 22px; container-type: inline-size; }
...
.farm-layout { display: grid; grid-template-columns: minmax(0, 1.5fr) minmax(330px, 1fr); gap: 24px; min-height: 0; flex: 1 0 auto; align-items: start; }
.farm-garden { position: relative; min-width: 0; padding: 18px 16px 14px; border: 1px solid #d1d9c4; border-radius: 16px; background: linear-gradient(#f0f2e5b8, #e8edddbd), url('/ui/farm/garden.webp') center top / cover; }
...
.farm-plots { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 13px; }
.farm-plot { position: relative; min-width: 0; min-height: 251px; display: flex; flex-direction: column; align-items: stretch; padding: 10px 10px 12px; border: 1px solid #becab1; border-radius: 14px; background: linear-gradient(#f6f6e8bd, #edf0deb0); box-shadow: 0 4px 14px #46633e09; }
...
.plot-earth { position: absolute; width: 90%; height: 36%; bottom: 3px; background: linear-gradient(170deg, #a7aa7c, #97966b); border: 2px solid #b9bb8b; border-radius: 45% 45% 40% 40%; box-shadow: 0 5px 0 #7c805c, 0 9px 9px #5b694821; transform: rotate(-3deg); overflow: hidden; }
.plot-earth i { width: 110%; height: 1px; background: #d5d1a035; position: absolute; top: 23%; left: -5%; transform: rotate(-12deg); }
.plot-earth i:nth-child(2) { top: 48%; }.plot-earth i:nth-child(3) { top: 73%; }
...
.plot-mature { border-color: #b6bd90; background: linear-gradient(#fbf6dd, #eff0d9); }
.plot-has-pest .plot-earth { filter: saturate(.7); }
.farm-plot.plot-expand { align-items: center; justify-content: center; gap: 7px; border: 1px dashed #b2bea1; background: #d8e2cd66; font-family: inherit; color: #758869; text-align: center; }
...
.farm-panel { min-width: 0; border: 1px solid #d0d8c1; background: #faf9efbf; border-radius: 14px; overflow: hidden; }
.farm-tabs { display: flex; border-bottom: 1px solid #dadfcf; background: #e8eddda8; padding: 3px 4px 0; }
...
.farm-tabs button.active { color: #416547; font-weight: 600; }.farm-tabs button.active::after { content: ''; position: absolute; bottom: 0; height: 3px; background: #718b60; left: 20%; right: 20%; border-radius: 4px 4px 0 0; }
.farm-tabs small { font-size: 8px; margin-left: 3px; padding: 1px 3px; color: #fff9e2; background: #a99962; border-radius: 8px; }
.farm-panel-content { max-height: 565px; overflow: auto; padding: 17px; scrollbar-width: thin; scrollbar-color: #bcc9ad transparent; }
...
.plot-action-effect { position: absolute; z-index: 3; top: 23%; left: 50%; width: max-content; max-width: 95%; transform: translateX(-50%); padding: 6px 9px; border-radius: 6px; font-size: 11px; color: #fff9e6; background: #78905dde; animation: farm-result-float 1.7s ease-out forwards; pointer-events: none; }.plot-action-effect i { position: absolute; width: 4px; height: 7px; border-radius: 50%; left: calc(var(--particle) * 13%); top: 25px; background: #dfbf6e; animation: farm-harvest-particle 1.2s ease-out forwards; animation-delay: calc(var(--particle) * 40ms); }.effect-water { background: #6b9fa4de; }.effect-water i { background: #85bac6; animation-name: farm-water-drop; }.effect-pest { background: #9a8a63de; }.effect-pest i { background: #a7b778; }.scene-water .plot-earth { background: #87886c; }
@keyframes farm-result-float { 0% { opacity: 0; margin-top: 10px; } 15%, 65% { opacity: 1; } 100% { opacity: 0; margin-top: -26px; } }
@keyframes farm-harvest-particle { from { transform: translateY(15px); opacity: 1; } to { transform: translate(calc((var(--particle) - 4) * 10px), -45px); opacity: 0; } }
@keyframes farm-water-drop { from { transform: translateY(-35px); opacity: 1; } to { transform: translateY(70px); opacity: 0; } }
@keyframes farm-loading-pulse { 50% { opacity: .4; transform: translateY(-5px); } }
@container (min-width: 1100px) { .farm-button { font-size: 13px; }.farm-owner strong { font-size: 16px; }.farm-owner span, .farm-level > span, .farm-resource > span { font-size: 12px; }.farm-level > small, .farm-resource small { font-size: 11px; }.garden-caption h3 { font-size: 23px; }.farm-plot { min-height: 271px; }.plot-scene { height: 124px; }.plot-scene :deep(.farm-plant) { width: 154px; height: 130px; }.plot-info strong { font-size: 16px; }.plot-info > span, .plot-actions .farm-button { font-size: 12px; }.plot-info > small, .plot-tended { font-size: 11px; }.farm-tabs button { font-size: 13px; }.panel-title h3 { font-size: 18px; }.panel-title > span { font-size: 11px; }.crop-shop-info h4 { font-size: 15px; }.crop-shop-info p { font-size: 12px; }.crop-shop-info > small, .crop-shop-info h4 span { font-size: 11px; }.crop-purchase label, .crop-purchase .farm-button { font-size: 12px; }.crop-purchase input { font-size: 13px; }.crop-shop-art { width: 75px; height: 83px; }.farm-garden-note { font-size: 12px; } }
@container (max-width: 900px) { .farm-layout { gap: 16px; grid-template-columns: minmax(0, 1fr) minmax(300px, .85fr); }.farm-plots { grid-template-columns: repeat(2, minmax(0, 1fr)); }.farm-summary { gap: 15px; grid-template-columns: minmax(160px, 1.2fr) 1fr .65fr .65fr; }.farm-panel-content { padding: 13px; }.crop-shop-art { width: 49px; }.farm-heading { gap: 12px; }.farm-heading-main { gap: 9px; }.farm-heading-actions { gap: 9px; } }
@container (max-width: 670px) { .farm-layout { grid-template-columns: 1fr; }.farm-plots { grid-template-columns: repeat(3, minmax(0, 1fr)); }.farm-panel-content { max-height: none; }.farm-summary { grid-template-columns: minmax(170px, 1.4fr) 1fr; gap: 15px 24px; }.farm-resource { display: flex; align-items: center; justify-content: space-between; }.farm-resource > strong { margin: 0; font-size: 19px; }.farm-heading h2 { font-size: 23px; }.farm-eyebrow { font-size: 8px; letter-spacing: 2px; }.farm-heading-main { flex-wrap: wrap; }.farm-heading-actions { flex-wrap: wrap; justify-content: flex-end; }.farm-heading .farm-back { padding-left: 0; }.farm-wallet { font-size: 10px; }.farm-wallet b { font-size: 15px; }.farm-plot { min-height: 237px; padding-inline: 8px; }.plot-scene { height: 101px; }.plot-scene :deep(.farm-plant) { height: 103px; }.crop-shop-art { width: 65px; }.crop-shop-info h4 span { float: none; margin-left: 7px; }.crop-gallery { grid-template-columns: repeat(3, minmax(0, 1fr)); }.farm-alert { flex-wrap: wrap; } }
@container (max-width: 430px) { .farm-plots { grid-template-columns: repeat(2, minmax(0, 1fr)); }.farm-heading { align-items: flex-start; }.farm-heading-main { gap: 7px; }.farm-heading-main > div { flex-basis: 100%; }.farm-summary { grid-template-columns: 1fr 1fr; gap: 13px; }.farm-owner { grid-column: 1 / -1; }.farm-level { grid-column: 1 / -1; }.crop-gallery { grid-template-columns: repeat(2, minmax(0, 1fr)); }.garden-caption h3 { font-size: 17px; }.farm-heading-actions { max-width: 45%; }.farm-heading { margin-bottom: 12px; } }
...
@media (prefers-reduced-motion: reduce) { .plot-action-effect, .plot-action-effect i, .loading-leaf { animation: none; }.farm-button, .farm-meter i, .plot-progress i { transition: none; } }
</style>
