...
import { roomInviteUrl, roomInviteDescription, type RoomInvite } from './roomInvites';
...
async function recruit() {
  ...
  catch (reason) { notice.value = reason instanceof Error ? reason.message : '当前客户端未能打开分享，请复制邀请链接或房间号发送给好友'; }
...
@container activity-viewport (max-height: 400px) { .room-invite { padding: 10px 16px; } header { margin-bottom: 7px; } h2 { font-size: 18px; } .invite-code { padding: 7px; } .room-invite p { margin: 8px 0; } }
</style>
