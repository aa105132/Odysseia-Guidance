...
async def generate_voice(
    ...
    **kwargs,
    ...
    使用场景：
    - 这个工具是「强约束的可选工具」，默认不要主动调用，不是每轮都必须用。
    - 只有当用户明确说“发语音”“语音回复”“念出来”等需求时，才应该调用。
    - 只有当你（月月）当前确实处于「极度愤怒 / 怒气爆发」的回击状态时，才允许主动调用。
    - 普通生气、轻微不爽、阴阳怪气、吐槽、问候、安慰、撒娇、闲聊，默认都不要主动调用。
    - 例外：当当前 provider 是小米 MiMo TTS 时，可以明显放宽主动发送语音的门槛；问候、安慰、闲聊、整活、角色扮演等场景都允许主动发语音，但搜索/总结/长结构化说明仍优先文字。
    - 如果你拿不准该不该发语音，按“不发”处理，直接文字回复。
    ...
    Args:
        ...
        emotion: （可选）情感风格。若当前 provider 是小米 MiMo，优先直接写自然中文风格短语，
            不受固定枚举限制，例如：温柔哄人、刚睡醒有点慵懒、压低声音撒娇、带点委屈地小声抱怨。
            豆包可用情感建议如下：
