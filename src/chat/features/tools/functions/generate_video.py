...
def _build_video_first_frame_prompt(
    ...
    *,
    ...
    common_constraints = (
        ...
        "不要多视角拼图，不要角色设定图，不要把同一角色画成正侧背三联图。"
    )
...
def _ensure_chinese_video_prompt(prompt: str, *, is_image_to_video: bool, duration: int) -> str:
    ...
    return (
        ...
        "整体保持电影感、写实质感和连续运镜；保持画面连续，避免画面跳变。"
    )
    ...
    **同样重要：图片请求不是视频请求。用户只是发图、要画图、改图、按图参考新画，或说"动画风格/动态姿势"时，禁止调用此工具。**
    ...
    Args:
        prompt: 视频描述提示词，必须使用中文自然语言分镜描述，禁止写成英文标签词或英文句子。
                ...
                **【视频提示词页 / 导演分镜工作流 - 生成前必须套用】**
                1) 先做导演规划，不要急着写单句提示词：
                   - 明确视频类型：文生视频 / 图生视频 / 长视频续写 / 图片先重绘首帧再动起来。
                   - 明确总时长、画幅、主体身份、场景、风格、是否需要声音或台词。
                   - 明确“整条视频的动作弧线”：开场建立 → 动作发展 → 情绪/镜头高潮 → 收束。

                2) 单段 prompt 必须像真实分镜，不要只写氛围词：
                   - 固定主体：角色外观、服装、饰品、身份、参考图关系。
                   - 固定场景：地点、时间、光源、前景/背景、环境质感。
                   - 主动作：走、转身、抬手、回头、奔跑、说话、挥手等。
                   - 二级动画：发丝、衣摆、尾巴、披风、烟雾、雨滴、光斑、尘埃、水面、窗外景物。
                   - 表情和视线：眨眼、微笑、惊讶、害羞、看向镜头、视线转移。
                   - 运镜和焦点：缓慢推进、拉远、横移、弧形环绕、手持微晃、低角度、过肩、焦点转移。
                   - 画面约束：保持角色身份稳定，避免变脸、肢体畸变、背景乱变和画面跳变。
                   禁止写成 “Cinematic / 3D realistic / vlog style”等英文堆词，要改成中文镜头语言：
                   “电影感写实三维动画、手持跟拍记录感、浅景深、柔和侧逆光、轻微镜头呼吸”。

                3) 单段 5-15 秒写法：
                   - 6秒：拆 3 个时间片，例如 0-2秒 / 2-4秒 / 4-6秒。
                   - 10秒：拆 4 个时间片，例如 0-2秒 / 2-5秒 / 5-8秒 / 8-10秒。
                   - 15秒：拆 5 个时间片，例如 0-3秒 / 3-6秒 / 6-9秒 / 9-12秒 / 12-15秒。
                   每个时间片只推进一个主动作，不要频繁切镜，不要把多个无关动作塞进同一秒。

                4) 长视频（超过15秒）必须使用 segment_prompts：
                   - 单个视频最长 15 秒，用户要 30秒/60秒/一分钟时，必须拆成多个 5-15 秒片段。
                   - 每一段都要单独写 prompt，不能用同一个 prompt 自动重复生成。
                   - 第 1 段负责建立主体、场景和动作起势。
                   - 第 2 段开始必须以“承接上一段尾帧……”开头，说明如何从上一段结尾继续。
                   - 每段结尾都要给下一段留连续动作，例如“手刚抬到半空”“镜头停在半侧身”“角色刚转向窗外”。
                   - 如果目标是 60 秒，推荐 4 段 × 15 秒；复杂剧情可 5-6 段，每段 10-12 秒。

                5) 图生视频判断：
                   - 用户说“把这张图动起来 / 保持原图构图 / 这张作为首帧”：
                     use_reference_image=True，prepare_video_first_frame=False，直接让原图自然动起来。
                   - 参考图只是头像、贴纸、设定图、表情、纯背景，或用户要换成广告/Vlog/剧情场景：
                     use_reference_image=True，prepare_video_first_frame=True，并填写 video_first_frame_prompt。
                   - 普通“参考图”不要乱写“首帧/尾帧”，除非用户明确要求帧控制。

                6) 好看的长视频结构示例：
                   - 第1段：建立环境和角色状态，动作很小，重点是稳定主体。
                   - 第2段：动作开始发展，镜头轻微推进或横移，加入二级动画。
                   - 第3段：动作/情绪达到最强，镜头更靠近，表情和光影更明显。
                   - 第4段：动作自然收束，画面停在适合循环或继续延长的姿态。

                7) 可直接生成这种单段中文自然语言提示词：
                  “基于参考图生成二次元动画视频：保持角色外观、服装、背景、构图和画风一致。0-2秒，角色背对镜头站定，发梢和披风被微风轻轻带动；2-4秒，镜头缓慢推进，角色肩膀微动并开始回头；4-6秒，角色看向镜头并眨眼，表情从平静变得灵动；6-8秒，镜头轻微弧形环绕，衣摆和发丝自然回弹；8-10秒，角色停在半侧身姿态，画面稳定收束，保持角色身份稳定，避免肢体畸变。”
        ...
        segment_prompts: （长视频必填）分段视频提示词列表。用户要求超过 15 秒时，月月必须为每一段写
                ...
                60秒示例：
                segment_durations=[15,15,15,15]
                segment_prompts=[
                  "第1段：建立场景……0-3秒……3-6秒……6-9秒……9-12秒……12-15秒……",
                  "第2段：承接上一段尾帧，角色刚抬起手……0-3秒……12-15秒停在适合继续的姿态……",
                  "第3段：承接上一段尾帧，镜头继续推进……",
                  "第4段：承接上一段尾帧，动作进入收束……"
                ]
