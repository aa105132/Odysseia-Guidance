...
@tool_metadata(
    name="总结",
    description="获取当前频道最近的聊天消息用于总结。仅在用户当前这条消息明确要求总结/概括/回顾对话时才使用。",
    emoji="📝",
    category="总结",
)
async def summarize_channel(
    ...
    **kwargs,
    ...
    """
    获取当前频道的最近消息列表并返回，供你进行总结。
    ⚠️ 严格使用条件：只有用户**当前这条消息**明确说出"总结"、"概括"、"回顾"等词时才调用。
    ❌ 禁止使用的情况：用户只是发了表情/图片/"嗯""哦""好"/无文字内容，即使上一条有人让你总结也不能用。
    ❌ 禁止连续多轮对话都调用总结——一次就够了，不要重复获取。

    [使用示例]
    - 用户说: "总结一下最近的对话" → `limit=200`
    - 用户说: "总结一下从昨天开始的对话" → `start_date="YYYY-MM-DD"`
