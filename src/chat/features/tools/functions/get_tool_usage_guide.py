    ...
    if normalized_topic in {"all", "image"}:
        image_guide: Dict[str, Any] = {
            ...
            "routing_rules": [
                ...
                "只有生成非频道用户的外部人物/同人角色/IP角色（现实人物、动漫/小说/游戏角色、明星、角色名等）或你确认不是本频道成员时，才调用 image_search 获取多张图片参考；多人物/多角色任务可以为每个人物分别调用 image_search，多次搜索结果会累计为本轮全局编号。之后必须由你分析并显式决定用哪张或哪几张图作参考。生成图片下一步必须调用 edit_image(image_search_reference_indexes=[...]) 一次性选择所有人物参考图，edit_prompt 只写保持参考图主体不变并修改动作/场景/构图，不要复述外观、服饰、发色、作品名或画风；禁止退回 generate_image 纯文生图。图生视频调用 generate_video(use_reference_image=true, image_search_reference_indexes=[...])。",
