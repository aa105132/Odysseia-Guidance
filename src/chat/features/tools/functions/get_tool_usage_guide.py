    ...
    if normalized_topic in {"all", "image"}:
        image_guide: Dict[str, Any] = {
            ...
            "routing_rules": [
                ...
                "只有生成非频道用户的外部人物/同人角色/IP角色（现实人物、动漫/小说/游戏角色、明星、角色名等）或你确认不是本频道成员时，才调用 image_search 获取多张图片参考；之后必须由你分析并显式决定用哪张或哪几张图作参考。生成图片下一步必须调用 edit_image(image_search_reference_index=... 或 image_search_reference_indexes=[...])，edit_prompt 只写保持参考图主体不变并修改动作/场景/构图，不要复述外观、服饰、发色、作品名或画风；禁止退回 generate_image 纯文生图。图生视频调用 generate_video(use_reference_image=true, image_search_reference_index=...)。",
                "如果用户说‘搜/找/给我看某人图片/参考图’，且对象不是频道成员/群友，调用 image_search 且 send_to_channel=true，把参考图发到频道；如果只是要生成，则 send_to_channel=false，仅内部参考。",
                "image_search 返回的图常带水印/平台文字；后续生成必须由你选择参考图，并用短 edit_prompt 保持参考图人物相似度；不要复制水印、署名、平台 UI、边框或文字。",
