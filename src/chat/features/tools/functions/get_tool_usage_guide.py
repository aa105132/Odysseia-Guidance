    ...
    if normalized_topic in {"all", "image"}:
        image_guide: Dict[str, Any] = {
            ...
            "routing_rules": [
                ...
                "只有生成非频道用户的外部人物/同人角色/IP角色（现实人物、动漫/小说/游戏角色、明星、角色名等）或你确认不是本频道成员时，才调用 image_search 获取多张图片参考；image_search 一次只能搜一个人物/角色/主体，禁止 [BATCH]、换行、||、逗号分隔、A和B 或任何多名字混搜。多人物混搜返回的图片无法可靠区分谁是谁，会导致后续图生图乱生成；多人物/多角色任务必须为每个人物分别调用 image_search，并记清每批全局编号对应哪个人物；最后选择参考图时必须从每个人物对应批次里各选至少 1 张，禁止只从同一人物/同一批里选多张来冒充不同人物。多次搜索结果会累计为本轮全局编号。之后必须由你分析并显式决定用哪张或哪几张图作参考。生成图片下一步必须调用 edit_image 或 edit_images_batch(image_search_reference_indexes=[...]) 一次性选择所有人物参考图，edit_prompt/edit_prompts 只写保持参考图主体不变并修改动作/场景/构图，不要复述外观、服饰、发色、作品名或画风；禁止退回 generate_image 纯文生图。图生视频调用 generate_video(use_reference_image=true, image_search_reference_indexes=[...])。",
