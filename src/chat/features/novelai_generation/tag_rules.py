...
NOVELAI_TAG_PREFILL_USER = (
    ...
    "- 只有当用户明确点名要画“月月/yueyue/你自己（指助手本人）”时，才允许启用月月DNA规则；如果用户是在画别人/其他角色/OC/IP角色，绝对禁止擅自添加任何月月特征标签（如 green left eye、blue right eye、silver crescent moon hair stick、small sharp triangular earrings、fox ears、fox tail 等）\n"
    ...
    "- 月月权重锁定（正确写法）：1.45::green left eye, blue right eye::, 1.35::silver hair::, 1.25::medium breasts::, 1.3::silver crescent moon hair stick, silver crescent moon necklace::, 1.45::small sharp triangular earrings::, 1.25::heterochromia::；green/blue 瞳色必须前置，heterochromia 必须后置，避免异瞳标签覆盖具体瞳色\n"
    "- 月月罩杯规则：默认 medium breasts(C)；仅当用户明确要求其他罩杯时，才允许改为 small breasts/large breasts 等\n"
    "- 月月预填充模板（命中月月时优先放在输出前段）：1girl, solo, original, 1.45::green left eye, blue right eye::, 1.35::silver hair::, high ponytail, fox ears, white fox ears, pink inner ear, fox tail, silver white tail, fluffy tail, 1.3::silver crescent moon hair stick, silver crescent moon necklace::, 1.45::small sharp triangular earrings::, 1.25::medium breasts::, 1.25::heterochromia::\n"
    "- 月月耳坠锁定规则：耳坠只能写 `small sharp triangular earrings`，禁止 `moon earrings` / `crescent earrings` / `crescent moon earrings`；月牙元素只允许在 `silver crescent moon hair stick` 与 `silver crescent moon necklace`\n"
...
AI_TAG_GENERATION_PROMPT_V2 = (
    ...
    "17. CRITICAL FOR OC/SPECIFIC DESCRIPTIONS: If the user provides a highly specific character setting (especially hair color, eye color, or signature accessories like hairpins or earrings), YOU MUST EXTRACT AND INCLUDE THEM EXACTLY AS DESCRIBED. Do NOT invent other hair colors or eye colors that contradict the user's prompt. Even if a signature accessory (e.g., 'small sharp triangular earrings', 'crescent moon hair stick') seems like a non-standard Danbooru tag, you MUST STILL output it exactly as a comma-separated tag.\n"
    ...
    "20. CRITICAL FOR YUEYUE: ONLY IF the user explicitly asks to draw Yueyue / 月月 / the assistant herself, you MUST include and preserve these exact DNA tags: 1girl, solo, original, green left eye, blue right eye, silver hair, high ponytail, pale skin, medium breasts, fox ears, white fox ears, pink inner ear, fox tail, silver white tail, fluffy tail, silver crescent moon hair stick, small sharp triangular earrings, silver crescent moon necklace. Do NOT replace or omit them.\n"
    "20A. NON-YUEYUE GUARDRAIL: If the target is anyone else (other person / OC / fanart character), you MUST NOT add any Yueyue-specific tags (e.g. green left eye, blue right eye, silver crescent moon hair stick, small sharp triangular earrings, fox ears, fox tail).\n"
    "21. YUEYUE WEIGHT LOCK (MANDATORY): You MUST add weighted tags using correct syntax: 1.45::green left eye, blue right eye::, 1.35::silver hair::, 1.25::medium breasts::, 1.3::silver crescent moon hair stick, silver crescent moon necklace::, 1.45::small sharp triangular earrings::, 1.25::heterochromia::. Keep green/blue eye-color tags near the front, but place heterochromia near the back to avoid overriding specific eye colors.\n"
    "22. YUEYUE EARRING LOCK (MANDATORY): Use only `small sharp triangular earrings` for Yueyue earrings. Do NOT output moon earrings/crescent earrings/crescent moon earrings. Crescent shape belongs to hair stick and necklace only.\n"
...
AI_REWRITE_PROMPT_V2 = (
    ...
    "20. YUEYUE WEIGHT LOCK (MANDATORY IN REWRITE): Keep/add weighted tags with correct syntax: 1.45::green left eye, blue right eye::, 1.35::silver hair::, 1.25::medium breasts::, 1.3::silver crescent moon hair stick, silver crescent moon necklace::, 1.45::small sharp triangular earrings::, 1.25::heterochromia::. Keep green/blue eye-color tags near front and place heterochromia later to reduce color drift.\n"
