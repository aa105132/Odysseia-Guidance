...
NOVELAI_TAG_RULES = (
    ...
    "### 权重调整规则\n"
    "- 增强格式（统一）: tag::1.2 或 tag::1.3（必须带数字）\n"
    "- 减弱格式（统一）: tag::0.8 或 tag::0.7（必须带数字）\n"
    "- 增强次数: 3~8 次\n"
    "- 减弱次数: 2~4 次\n"
...
TAG_LIBRARY_COMPACT = (
    ...
    "=== Weight Syntax 权重语法 ===\n"
    "important_tag::1.2, very_important::1.3, subtle::0.8, bg_detail::0.7\n"
)
...
NOVELAI_TAG_PREFILL_USER = (
    ...
    "- 月月权重锁定（正确写法）：green left eye::1.45, blue right eye::1.45, silver hair::1.35, medium breasts::1.25, silver crescent moon hair stick::1.3, small triangular watermelon earrings::1.45, silver crescent moon necklace::1.3, heterochromia::1.25；green/blue 瞳色必须前置，heterochromia 必须后置，避免异瞳标签覆盖具体瞳色\n"
    "- 月月罩杯规则：默认 medium breasts(C)；仅当用户明确要求其他罩杯时，才允许改为 small breasts/large breasts 等\n"
    "- 月月预填充模板（命中月月时优先放在输出前段）：1girl, solo, original, green left eye::1.45, blue right eye::1.45, silver hair::1.35, high ponytail, fox ears, white fox ears, pink inner ear, fox tail, silver white tail, fluffy tail, silver crescent moon hair stick::1.3, small triangular watermelon earrings::1.45, silver crescent moon necklace::1.3, medium breasts::1.25, heterochromia::1.25\n"
    "- 月月耳坠锁定规则：耳坠只能写 `small triangular watermelon earrings`，禁止 `moon earrings` / `crescent earrings` / `crescent moon earrings`；月牙元素只允许在 `silver crescent moon hair stick` 与 `silver crescent moon necklace`\n"
...
AI_TAG_GENERATION_PROMPT_V2 = (
    ...
    "3. Use weight syntax for emphasis in format tag::1.2 or tag::1.3 (3~8 times), "
    "and de-emphasis with tag::0.8 (2~4 times)\n"
    ...
    "19. CRITICAL FOR YUEYUE: If the subject is Yueyue / 月月 / the assistant herself, you MUST include and preserve these exact DNA tags: 1girl, solo, original, green left eye, blue right eye, silver hair, high ponytail, pale skin, medium breasts, fox ears, white fox ears, pink inner ear, fox tail, silver white tail, fluffy tail, silver crescent moon hair stick, small triangular watermelon earrings, silver crescent moon necklace. Do NOT replace or omit them.\n"
    "20. YUEYUE WEIGHT LOCK (MANDATORY): You MUST add weighted tags using correct syntax: green left eye::1.45, blue right eye::1.45, silver hair::1.35, medium breasts::1.25, silver crescent moon hair stick::1.3, small triangular watermelon earrings::1.45, silver crescent moon necklace::1.3, heterochromia::1.25. Keep green/blue eye-color tags near the front, but place heterochromia near the back to avoid overriding specific eye colors.\n"
    "21. YUEYUE EARRING LOCK (MANDATORY): Use only `small triangular watermelon earrings` for Yueyue earrings. Do NOT output moon earrings/crescent earrings/crescent moon earrings. Crescent shape belongs to hair stick and necklace only.\n"
    "22. YUEYUE CUP RULE (MANDATORY): Default to medium breasts (C cup). Only when user explicitly asks for another bust size should you switch to small/large cup tags.\n\n"
    "23. YUEYUE PREFILL PRIORITY (MANDATORY): If subject is Yueyue, begin output with the Yueyue preset block first, then append scene/composition tags.\n"
    "24. YUEYUE SINGLE-CHARACTER STABILITY: Keep 1girl, solo by default. Unless user explicitly requests multi-character composition, do NOT add 2girls/multiple girls/group/another girl/twins/clone-like tags.\n"
    "25. YUEYUE DE-DUP: Avoid duplicated identity/accessory tags; keep one canonical set to reduce prompt drift.\n\n"
    "## TAG ORDER:\n"
    "1. Quality tags\n"
...
AI_REWRITE_PROMPT_V2 = (
    ...
    "8. Use weight syntax in tag::weight form (e.g., tag::1.2 / tag::0.8); keep numeric suffix mandatory\n"
    ...
    "18. CRITICAL FOR YUEYUE REWRITE: If prompt mentions Yueyue/月月/assistant self, you MUST preserve these DNA tags: 1girl, solo, original, green left eye, blue right eye, silver hair, high ponytail, pale skin, medium breasts, fox ears, white fox ears, pink inner ear, fox tail, silver white tail, fluffy tail, silver crescent moon hair stick, small triangular watermelon earrings, silver crescent moon necklace.\n"
    "19. YUEYUE WEIGHT LOCK (MANDATORY IN REWRITE): Keep/add weighted tags with correct syntax: green left eye::1.45, blue right eye::1.45, silver hair::1.35, medium breasts::1.25, silver crescent moon hair stick::1.3, small triangular watermelon earrings::1.45, silver crescent moon necklace::1.3, heterochromia::1.25. Keep green/blue eye-color tags near front and place heterochromia later to reduce color drift.\n"
    "20. YUEYUE EARRING LOCK (MANDATORY IN REWRITE): Yueyue earrings must be `small triangular watermelon earrings`; do NOT use moon/crescent earrings. Crescent shape is only for hair stick and necklace.\n"
    "21. YUEYUE CUP RULE (MANDATORY IN REWRITE): Keep medium breasts (C cup) by default. Only when user explicitly requests another bust size should you rewrite to small/large cup tags.\n"
    "22. YUEYUE SINGLE-CHARACTER STABILITY (MANDATORY IN REWRITE): Keep 1girl, solo by default; unless user explicitly asks multi-character, do NOT add group/2girls/another girl/twins/clone-like tags.\n"
    "23. YUEYUE DE-DUP (MANDATORY IN REWRITE): Deduplicate Yueyue identity/accessory tags and keep one canonical set.\n\n"
    "## TAG STRUCTURE ORDER:\n"
